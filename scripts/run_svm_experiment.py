...
def train_and_evaluate(
    ...
    grid = GridSearchCV(
        ...
        param_grid={"model__C": [0.25, 1.0, 4.0]},
    ...
    print("[3/6] Training Linear SVM with 3-fold CV on the 80% training split")
    ...
    metrics = {
        "random_state": RANDOM_STATE,
        "model": "LinearSVC",
        "feature_count_raw": len(NUMERIC_FEATURES) + len(CATEGORICAL_FEATURES),
...
def save_model_comparison(cv_results: pd.DataFrame, path: Path) -> None:
    ...
    labels = [f"C={c}" for c in df["param_model__C"]]
    x = np.arange(len(df))
    width = 0.35
    fig, ax = plt.subplots(figsize=(8, 5), dpi=150)
...
其中 `C` 是正则化参数，控制间隔宽度与训练误差之间的权衡。本实验在训练集上使用 3 折交叉验证比较 `C = 0.25, 1.0, 4.0`，最终最优参数为 `C = {best_c}`。实验流程图可放入 `outputs/figures/10_experiment_workflow.png`，SVM 算法流程图可放入 `outputs/figures/11_svm_workflow.png`。
