...
其中 `C` 是正则化参数，控制间隔宽度与训练误差之间的权衡。本实验在训练集上使用 3 折交叉验证比较 `C = 0.02, 0.03, 0.05, 0.1, 0.25, 0.5`，并同时比较 `class_weight=None` 与 `class_weight="balanced"`，最终最优参数为 `C = 0.02`、`class_weight = balanced`。实验流程图可放入 `outputs/figures/10_experiment_workflow.png`，SVM 算法流程图可放入 `outputs/figures/11_svm_workflow.png`。
...
- Weighted Recall: 0.6413
- Weighted F1: 0.6411
- 3 折交叉验证最优 Weighted F1: 0.6381
...
- 起点空间网格（`start_grid`）：置换后准确率平均下降 0.0289
- 估算骑行速度（`speed_kmh`）：置换后准确率平均下降 0.0232
- 距离分箱（`distance_bin`）：置换后准确率平均下降 0.0198
- 起终点站点编号组合（`route_station_id`）：置换后准确率平均下降 0.0191
- 终点空间网格（`end_grid`）：置换后准确率平均下降 0.0182
- 终点站点是否缺失（`end_station_missing`）：置换后准确率平均下降 0.0178
- 自行车类型（`rideable_type`）：置换后准确率平均下降 0.0175
- 终点站点编号（`end_station_id`）：置换后准确率平均下降 0.0134
